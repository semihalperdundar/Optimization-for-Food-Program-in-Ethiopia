...
Original file is located at
    https://colab.research.google.com/drive/10pO1XokUYs_XoSkih-eBYmrE0qj72vVq
"""
...
# Normalizasyon: isimleri büyük harfe çevirip sondaki "S" karakterini kaldırıyoruz
...
# Normalizasyon: commodity isimlerini aynı standarda çekiyoruz
...
# Nutritional matrix: 100 gram değerlerini ton başına çevirmek için 10000 ile çarpıyoruz.
...
# ---------------- Integrated LP Formulation ----------------
# Karar değişkenleri:
# x: Tedarikçi -> Liman, sea_flow: Limanlar arası, y: Liman -> Depo, z: Depo -> Kamp
...
for p in range(n_ports):
    for f in range(n_commodities_proc):
        constraint_groups["port_flow_balance"].append(cp.sum(x[:, p, f]) == cp.sum(sea_flow[p, :, f]))

# (C) Liman Dengesi: Deniz -> Depo
constraint_groups["port_balance"] = []
for d in range(n_ports):
    for f in range(n_commodities_proc):
        constraint_groups["port_balance"].append(cp.sum(sea_flow[:, d, f]) == cp.sum(y[d, :, f]))
...
for w, wh_name in enumerate(warehouses):
    if wh_name in warehouse_cap_dict and warehouse_cap_dict[wh_name] > 0:
        constraint_groups["warehouse_balance"].append(cp.sum(z[w, :, :]) <= warehouse_cap_dict[wh_name])
    for f in range(n_commodities_proc):
        constraint_groups["warehouse_balance"].append(cp.sum(y[:, w, f]) == cp.sum(z[w, :, f]))
...
for p, port_name in enumerate(ports):
    ...
    if port_cap > 0:
        constraint_groups["port_capacity"].append(cp.sum(x[:, p, :]) <= port_cap)

# (F) Kamp Talep Kısıtı (Beneficiaries)
# Burada, her kampın toplam dağıtımının (z üzerinden) hem
# - LP modelindeki dağıtım maliyetleri hem de
# - toplam gıda ihtiyacının (hem nutrient hem de toplam miktar açısından) karşılanması sağlanır.
...
for c, camp_name in enumerate(beneficiary_camps):
    ...
    # Kamp talebi, kamp nüfusu ile çarpılmış toplam gıda miktarı olarak formüle edilebilir.
    # Bu kısıt, dağıtım akışının (z) kampın toplam gıda ihtiyacını (belirli bir min değeri) karşılamasını zorunlu kılar.
    # Örneğin: Her birey için minimum gıda miktarı olarak min_food_per_person belirlenebilir.
    # assignment kapsamında doğrudan nutrient gereksinimi öne çıktığından, burada doğrudan toplam dağıtım da
    # (z'nin toplamı) kamp talebini karşılamalı.
    min_food_per_person = 0.0  # Eğer her commodity'nin kullanılmasını zorunlu kılmak istemiyorsanız;
                                # gereksinim sadece nutrient kısıtlarıyla sağlanıyor.
    base_demand = camp_population * min_food_per_person
    slack = cp.Variable(nonneg=True, name=f"demand_slack_{c}")
    demand_slacks[c] = slack
    # Burada, toplam dağıtım (tüm commodity'ler) + slack >= base_demand
    constraint_groups["camp_demand"].append(cp.sum(z[:, c, :]) + slack >= base_demand)
    # Not: Eğer ekstra toplam gıda miktarı gereksinimi varsa, bu kısıtı güncellemeniz gerekebilir.

# (G) Nutrient Kısıtları: Her kamp için tüm nutrient gereksinimlerinin sağlanması
constraint_groups["nutritional"] = []
# new_nutr_matrix: her procurement commodity için, nutrient değerleri (nutr_matrix) kullanılıyor.
...
for c in range(n_camps):
    camp_population = beneficiaries_dict.get(beneficiary_camps[c], 0)
    for j, nutr_name in enumerate(nutr_list_nutr):
        ...
        # Nutrient kısıtı: Her kamp için, tüm commodity'lerden sağlanan nutrient miktarı,
        # slack ile birlikte, günlük gereksinimi karşılamalı.
        constraint_groups["nutritional"].append(
            cp.sum(cp.multiply(z[:, c, :], new_nutr_matrix[:, j].reshape(1, -1))) + slack_nutr >= base_req
        )

# Tüm kısıtları tek bir listeye ekleyelim:
all_constraints = []
...
result_lp = lp_problem.solve(solver=cp.GLPK_MI, verbose=True)
print("\nLP Solver Status:", lp_problem.status)
print("LP Optimal Total Cost (with penalties):", lp_problem.value)
...
def diagnose_constraint_groups(objective, constraint_groups):
    ...
    for group in constraint_groups.values():
        all_constraints += group
    print("\nToplam kısıt sayısı:", len(all_constraints))
...
(CVXPY) Mar 04 03:45:50 PM: Your problem has 4976 variables, 920 constraints, and 2 parameters.
(CVXPY) Mar 04 03:45:50 PM: It is compliant with the following grammars: DCP, DQCP
(CVXPY) Mar 04 03:45:50 PM: CVXPY will first compile your problem; then, it will invoke a numerical solver to obtain a solution.
(CVXPY) Mar 04 03:45:50 PM: Your problem is compiled with the CPP canonicalization backend.
-------------------------------------------------------------------------------
                                  Compilation                                  
-------------------------------------------------------------------------------
(CVXPY) Mar 04 03:45:50 PM: Compiling problem (target solver=GLPK_MI).
(CVXPY) Mar 04 03:45:50 PM: Reduction chain: Dcp2Cone -> CvxAttr2Constr -> ConeMatrixStuffing -> GLPK_MI
(CVXPY) Mar 04 03:45:50 PM: Applying reduction Dcp2Cone
...
(CVXPY) Mar 04 03:45:50 PM: Applying reduction CvxAttr2Constr
(CVXPY) Mar 04 03:45:51 PM: Applying reduction ConeMatrixStuffing
(CVXPY) Mar 04 03:45:55 PM: Applying reduction GLPK_MI
(CVXPY) Mar 04 03:45:55 PM: Finished problem compilation (took 5.397e+00 seconds).
(CVXPY) Mar 04 03:45:55 PM: (Subsequent compilations of this problem, using the same arguments, should take less time.)
-------------------------------------------------------------------------------
                                Numerical solver                               
-------------------------------------------------------------------------------
(CVXPY) Mar 04 03:45:55 PM: Invoking solver GLPK_MI  to obtain a solution.
-------------------------------------------------------------------------------
                                    Summary                                    
-------------------------------------------------------------------------------
(CVXPY) Mar 04 03:45:56 PM: Problem status: optimal
(CVXPY) Mar 04 03:45:56 PM: Optimal value: 3.648e+07
(CVXPY) Mar 04 03:45:56 PM: Compilation took 5.397e+00 seconds
(CVXPY) Mar 04 03:45:56 PM: Solver (including time spent in interface) took 6.578e-01 seconds

LP Solver Status: optimal
LP Optimal Total Cost (with penalties): 36481424.80649788